...
#define   DEF_AVI_FILENAME          "A:movie.avi"
    ...
    avi_t *avi;
    S_JPEG_CTX ctx = {0};
    lv_obj_t *img = (lv_obj_t *)pdata;

    // AVI-MJPEG Player
    if ((avi = AVI_open_input_file(DEF_AVI_FILENAME, 1)) == NULL)
    {
        sysprintf("[%s]Failed to open the avi file - %s\n", __func__, DEF_AVI_FILENAME);
    }
    else
    {
        int key = 0;
        ...
        do
        {
            ...
            {
                key = 0;
                ctx.m_u32SrcBufLen = AVI_read_frame(avi, ctx.m_pvSrcBufAddr, &key);
                idx++;
                if (ctx.m_u32SrcBufLen < 0)
                {
                    sysprintf("Failed to read AVI frame.\n");
                    goto _quit_read;
                }
                else if (!key || (ctx.m_u32SrcBufLen == 0))
                {
                    continue;
                }
...
#warning "NotSupport!!"
                ...
                //sysprintf("[%d/%d]framesize: %d, key: %d crc32: 0x%08x\n", idx+1, AVI_video_frames(avi), ctx.m_u32SrcBufLen, key, crc32(ctx.m_pvSrcBufAddr, ctx.m_u32SrcBufLen));
