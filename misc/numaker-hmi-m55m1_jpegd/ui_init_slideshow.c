/**
 * @file ui_init_slideshow.c
 * @brief Simple slideshow implementation using LVGL
 *
 * This file scans for JPG images in the root directory (A:/) and displays
 * them in a slideshow format with a 5-second interval.
 */
#include "lvgl.h"
#include <string.h>
...
// Helper function to check if a file is a JPG/JPEG
...
// Scan A:/ for JPG files and store them in the list
static void scan_jpg_files(void) {
  ...
  while (lv_fs_dir_read(&dir, fn, sizeof(fn)) == LV_FS_RES_OK) {
    ...
    if (is_jpg(fn) && jpg_count < MAX_JPG_FILES) {
      lv_snprintf(jpg_list[jpg_count], sizeof(jpg_list[0]), "A:/%s", fn);
      jpg_count++;
    }
  }
...
// Timer callback to switch to the next image
...
void ui_init(void) {
  ...
  slide_timer = lv_timer_create(slide_next_cb, SLIDE_INTERVAL, NULL);
}
