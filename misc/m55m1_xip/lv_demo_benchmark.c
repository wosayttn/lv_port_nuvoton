...
void lv_demo_benchmark_summary_display(const lv_demo_benchmark_summary_t * summary)
{
    ...
    /* csv log */
    printf("Benchmark Summary (%d.%d.%d %s)\r\n",
           LVGL_VERSION_MAJOR,
           LVGL_VERSION_MINOR,
           LVGL_VERSION_PATCH,
           LVGL_VERSION_INFO);
    printf("Name, Avg. CPU, Avg. FPS, Avg. time, render time, flush time\r\n");
    ...
    for(size_t i = 0; scenes[i].create_cb; i++) {
        ...
        if(scenes[i].measurement_cnt == 0) {
            ...
            /* csv log */
            printf("%s, %"LV_PRIu32"%%, %"LV_PRIu32", %"LV_PRIu32", %"LV_PRIu32", %"LV_PRIu32"\r\n",
                   scenes[i].name,
    ...
    if(summary->valid_scene_cnt < 1) {
        ...
        lv_table_set_cell_value_fmt(table, 1, 3, "%"LV_PRIu32" ms (%"LV_PRIu32" + %"LV_PRIu32")",
                                    total_time, render_time, flush_time);
        /* csv log */
        printf("All scenes avg.,%"LV_PRIu32"%%, %"LV_PRIu32", %"LV_PRIu32", %"LV_PRIu32", %"LV_PRIu32"\r\n",
