...
static size_t avi_seek(lv_fs_file_t *fd, uint32_t pos, lv_fs_whence_t whence)
{

    lv_fs_res_t res = lv_fs_seek(fd, pos, whence);

    if (res == LV_FS_RES_OK)
        res = lv_fs_tell(fd, &pos);

    return (res == LV_FS_RES_OK) ? (int)pos : -1;
}
...
static int avi_add_chunk(avi_t *AVI, unsigned char *tag, unsigned char *data, int length)
{
    ...
    {
        avi_seek(AVI->fdes, AVI->pos, SEEK_SET);
...
#define AVIF_COPYRIGHTED        0x00020000
    ...
    if (avi_seek(AVI->fdes, 0, SEEK_SET) < 0 ||
            avi_write(AVI->fdes, (char *)AVI_header, HEADERBYTES) != HEADERBYTES ||
            avi_seek(AVI->fdes, AVI->pos, SEEK_SET) < 0)
    {
...
//   nhb += MAX_INFO_STRLEN;
    ...
    if (avi_seek(AVI->fdes, 0, SEEK_SET) < 0 ||
            avi_write(AVI->fdes, (char *)AVI_header, HEADERBYTES) != HEADERBYTES ||
            lv_fs_ftruncate(AVI->fdes, AVI->pos) < 0)
    {
        AVI_errno = AVI_ERR_CLOSE;
        return -1;
    }
...
int AVI_append_audio(avi_t *AVI, char *data, long bytes)
{
    ...
    //update chunk header
    avi_seek(AVI->fdes, pos + 4, SEEK_SET);
    long2str(c, length + bytes);
    avi_write(AVI->fdes, (char *)c, 4);

    avi_seek(AVI->fdes, pos + 8 + length, SEEK_SET);
...
int avi_parse_input_file(avi_t *AVI, int getIndex)
{
    ...
    while (1)
    {
        ...
        if (strncasecmp(data, "LIST", 4) == 0)
        {
            ...
            if (strncasecmp(data, "hdrl", 4) == 0)
            {
                ...
                // offset of header
                header_offset = avi_seek(AVI->fdes, 0, SEEK_CUR);

                if (avi_read(AVI->fdes, (char *)hdrl_data, n) != n) ERR_EXIT(AVI_ERR_READ)
                }
            else if (strncasecmp(data, "movi", 4) == 0)
            {
                AVI->movi_start = avi_seek(AVI->fdes, 0, SEEK_CUR);
                avi_seek(AVI->fdes, n, SEEK_CUR);
            }
            else
                avi_seek(AVI->fdes, n, SEEK_CUR);
        }
                    ...
                    else if (lasttag == 2)
                    {
                        ...
                        if (wfe != NULL)
                        {
                            ...
                            if (wfe->cb_size != 0)
                            {
                                ...
                                if (nwfe != 0)
                                {
                                    off_t lpos = avi_seek(AVI->fdes, 0, SEEK_CUR);
                                    avi_seek(AVI->fdes, header_offset + i + sizeof(WAVEFORMATEX_avilib), SEEK_SET);
                                    wfe = (WAVEFORMATEX_avilib *)nwfe;
                                    nwfe = &nwfe[sizeof(WAVEFORMATEX_avilib)];
                                    avi_read(AVI->fdes, nwfe, wfe->cb_size);
                                    avi_seek(AVI->fdes, lpos, SEEK_SET);
                                }
                            }
    ...
    avi_seek(AVI->fdes, AVI->movi_start, SEEK_SET);
    ...
    if (AVI->idx)
    {
        ...
        avi_seek(AVI->fdes, pos, SEEK_SET);
        if (avi_read(AVI->fdes, data, 8) != 8) ERR_EXIT(AVI_ERR_READ)
            ...
            {
                avi_seek(AVI->fdes, pos + AVI->movi_start - 4, SEEK_SET);
                if (avi_read(AVI->fdes, data, 8) != 8) ERR_EXIT(AVI_ERR_READ)
                    if (strncasecmp(data, (char *)AVI->idx[i], 4) == 0 && str2ulong((unsigned char *)data + 4) == len)
                    {
        ...
        while (1)
        {
            ...
            if (((data[2] == 'd' || data[2] == 'D') &&
                    ...
                    || ((data[2] == 'w' || data[2] == 'W') &&
                        (data[3] == 'b' || data[3] == 'B')))
            {
                avi_add_index_entry(AVI, (unsigned char *)data, 0, avi_seek(AVI->fdes, 0, SEEK_CUR) - 8, n);
            }

            avi_seek(AVI->fdes, PAD_EVEN(n), SEEK_CUR);
        }
        idx_type = 1;
    }
...
long AVI_read_frame(avi_t *AVI, char *vidbuf, int *keyframe)
{
    ...
    *keyframe = (AVI->video_index[AVI->video_pos].key == 0x10) ? 1 : 0;
    ...
    avi_seek(AVI->fdes, AVI->video_index[AVI->video_pos].pos, SEEK_SET);
                  ...
                  long *len)
{
    ...
    while (1)
    {
        ...
        else if (avi_seek(AVI->fdes, n, SEEK_CUR) < 0)  return 0;
    }
}
