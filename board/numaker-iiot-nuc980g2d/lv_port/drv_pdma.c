...
static void nu_pdma_init(void)
{
    ...
    sysInstallISR(IRQ_LEVEL_7, (IRQn_Type)nu_pdma_arr[PDMA0_IDX].eIRQn, PDMA0_IRQHandler);
    sysInstallISR(IRQ_LEVEL_7, (IRQn_Type)nu_pdma_arr[PDMA1_IDX].eIRQn, PDMA1_IRQHandler);
    ...
    /* Writeback data in dcache to memory before transferring. */
    {
        ...
        while (next != NULL)
        {
            ...
            if ((u32SrcCtl == PDMA_SAR_INC)) // for M2P, M2M
                sysCleanInvalidatedDcache((uint32_t)next->SA, u32FlushLen);

            /* Flush Dst buffer into memory. */
            if ((u32DstCtl == PDMA_DAR_INC)) // for P2M, M2M
                sysCleanInvalidatedDcache((uint32_t)next->DA, u32FlushLen);

            /* Flush descriptor into memory */
            if (!((uint32_t)next & BIT31))
                sysCleanInvalidatedDcache((uint32_t)next, sizeof(DSCT_T));
            ...
            if (bNonCacheAlignedWarning)
            {
                if ((u32FlushLen & (DEF_CACHE_LINE_SIZE - 1)) ||
                        ...
                        ((uint32_t)next & (DEF_CACHE_LINE_SIZE - 1)))
                {
                    /*
...
static int nu_pdma_memfun_employ(void)
{
    ...
    //        ((result = rt_sem_take(nu_pdma_memfun_actor_pool_sem, RT_WAITING_FOREVER)) == 0))
    {
        //LV_ASSERT(result == 0);
