...
static bool lv_draw_n9h26_blt_blend_fill(lv_color_t *dest_buf, const lv_area_t *dest_area, const lv_area_t *fill_area, lv_color_t color);
static bool lv_draw_n9h26_blt_blend_map(lv_color_t *dest_buf, const lv_area_t *dest_area, lv_color_t *src_buf, lv_coord_t src_stride, const lv_area_t *src_area, lv_opa_t opa);
...
void lv_draw_n9h26_blt_blend(lv_draw_ctx_t *draw_ctx, const lv_draw_sw_blend_dsc_t *dsc)
{
            ...
            (dsc->blend_mode == LV_BLEND_MODE_NORMAL))
    {
        if (dsc->src_buf)
        {
            ...
            done = lv_draw_n9h26_blt_blend_map(draw_ctx->buf, draw_ctx->buf_area, (lv_color_t *)src_buf, (lv_coord_t)lv_area_get_width(dsc->blend_area), &blend_area, dsc->opa);
        }
        else if (dsc->opa >= LV_OPA_MAX) //Fill
        {
            lv_area_move(&blend_area, -draw_ctx->buf_area->x1, -draw_ctx->buf_area->y1);

            done = lv_draw_n9h26_blt_blend_fill(draw_ctx->buf, draw_ctx->buf_area, &blend_area, dsc->color);
        }
    }
