...
{
    SCB_CleanDCache_by_Addr(px_map, (uint32_t)lv_area_get_size(area) * (LV_COLOR_DEPTH / 8));
...
static void lv_port_disp_partial(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
    ...
    uint16_t *pDisp = (uint16_t *)psLCDInfo->pvVramStartAddr + (LV_HOR_RES_MAX * area->y1 + area->x1);
    ...
    for (y = 0; y < h; y++)
    {
        ...
        SCB_CleanDCache_by_Addr(pDisp, (uint32_t)w * (LV_COLOR_DEPTH / 8));
...
#else
    LV_LOG_INFO("Use One screen-size shadow buffer, 0x%08x, 0x%08x.", buf2, buf3);

    lv_display_set_flush_cb(disp, lv_port_disp_partial);               /*Set a flush callback to draw to the display*/
    lv_display_set_buffers(disp, buf2, NULL, u32FBSize, LV_DISPLAY_RENDER_MODE_PARTIAL); /*Set an initialized buffer*/
#endif
}
