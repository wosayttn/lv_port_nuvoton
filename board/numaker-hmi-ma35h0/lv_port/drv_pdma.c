...
#define PDMA_CH_Msk                 ((1u<<PDMA_CH_MAX)-1)
...
#define NU_PDMA_GET_BASE(ch)   (PDMA_T *)((((ch)/PDMA_CH_MAX)*0x10000U) + DEF_PDMA_BASE_START)
...
static void PDMA0_IRQHandler(void);
static void PDMA1_IRQHandler(void);
...
static void nu_pdma_init(void)
{
    ...
    IRQ_SetHandler((IRQn_ID_t)nu_pdma_arr[PDMA0_IDX].eIRQn, PDMA0_IRQHandler);
    IRQ_SetHandler((IRQn_ID_t)nu_pdma_arr[PDMA1_IDX].eIRQn, PDMA1_IRQHandler);

    IRQ_Enable((IRQn_ID_t)PDMA0_IRQn);
    ...
    for (i = (PDMA_START + 1); i < PDMA_CNT; i++)
    {
        ...
        /* Enable IP clock */
        CLK_EnableModuleClock(nu_pdma_arr[i].u32ClkEnId);
        ...
        IRQ_Enable((IRQn_ID_t)nu_pdma_arr[i].eIRQn);
    }
...
static int nu_pdma_timeout_set(int i32ChannID, int i32Timeout_us)
{
    ...
    if (i32Timeout_us && (u32ModChannId < PDMA_CH_MAX))    // Limit
    {
...
static void _nu_pdma_transfer(int i32ChannID, uint32_t u32Peripheral, nu_pdma_desc_t head, uint32_t u32IdleTimeout_us)
{
    ...
    if (1)
    {
        ...
        while (next != NULL)
        {
            ...
            {
                if ((u32FlushLen & (DEF_CACHE_LINE_SIZE - 1)) ||
                        (next->SA & (DEF_CACHE_LINE_SIZE - 1)) ||
                        (next->DA & (DEF_CACHE_LINE_SIZE - 1)) ||
                        (ptr_to_u32(next) & (DEF_CACHE_LINE_SIZE - 1)))
                {
                    /*
            ...
            next = (nu_pdma_desc_t)addr_s(next->NEXT);
...
void PDMA_IRQHandler(PDMA_T *PDMA)
{
    ...
    {
        int module_id = (ptr_to_u32(PDMA) - DEF_PDMA_BASE_START) / 0x10000UL;
...
int nu_pdma_memfun(void *dest, void *src, uint32_t u32DataWidth, unsigned int u32TransferCnt, nu_pdma_memctrl_t eMemCtl)
{
...
void *nu_pdma_memcpy(void *dest, void *src, unsigned int count)
{
    ...
    {
        uint32_t u32src   = ptr_to_u32(src) + u32Offset;
        uint32_t u32dest  = ptr_to_u32(dest) + u32Offset;
                ...
                (NVT_ALIGN_DOWN(u32Remaining, i) >= i))
        {
            ...
            if (u32TXCnt != nu_pdma_memfun(ptr_s(u32dest), ptr_s(u32src), i * 8, u32TXCnt, eMemCtl_SrcInc_DstInc))
