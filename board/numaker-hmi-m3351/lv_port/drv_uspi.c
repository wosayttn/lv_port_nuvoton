...
int nu_uspi_send_then_recv(USPI_T *base, const uint8_t *tx, int tx_len, uint8_t *rx, int rx_len, int dw)
{
    USPI_SET_SS_LOW(base);
    ...
    if (rx)
    {
        ...
        {
            /* Input data to SPI TX FIFO */
            rx_len -= nu_uspi_write(base, (const uint8_t *)&dummy, dw);
            while (USPI_IS_BUSY(base));
            if (!USPI_GET_RX_EMPTY_FLAG(base))
                rx += nu_uspi_read(base, rx, dw);
        }
    }

    USPI_SET_SS_HIGH(base);
