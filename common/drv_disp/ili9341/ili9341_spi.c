...
static int nu_spi_read(SPI_T *spi, uint8_t *rx)
{
...
static int nu_spi_write(SPI_T *spi, const uint8_t *tx)
{
    int dw = SPI_GET_DATA_WIDTH(spi) / 8;
...
static int nu_spi_transmit_poll(SPI_T *spi, const uint8_t *tx, uint8_t *rx, int length)
{
    int dw = SPI_GET_DATA_WIDTH(spi) / 8;
    ...
    else if ((tx == NULL) && (rx != NULL))
    {
        ...
        while (length > 0)
        {
            ...
            length -= nu_spi_write(spi, (const uint8_t *)&s_u32Dummy);

            /* Read data from RX FIFO */
            rx += nu_spi_read(spi, rx);
        }
    } // else if (tx == NULL && rx != NULL)
    // Read&Write
    else
...
static int nu_spi_transmit_pdma(SPI_T *spi, const void *tx, void *rx, int length)
{
    // TODO
    return nu_spi_transmit_poll(spi, tx, rx, length);
}
#endif
...
static int nu_spi_transfer(SPI_T *spi, const void *tx, void *rx, int length)
{
    ...
    if (!((uint32_t)tx % dw) &&
        ...
        ret = nu_spi_transmit_pdma(spi, tx, rx, length);
    else
#endif
	      ret = nu_spi_transmit_poll(spi, tx, rx, length);
