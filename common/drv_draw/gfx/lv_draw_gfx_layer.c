...
void lv_draw_gfx_layer(lv_draw_task_t *t)
{
    ...
    /* It can happen that nothing was drawn on a layer and therefore its buffer is
     * not allocated. In this case just return.
     */
    if (draw_buf == NULL)
        return;

#if 0
    sysprintf("[GFX 2D] Layer: w=%d, h=%d\n", (int)draw_buf->header.w, (int)draw_buf->header.h);
#endif

    const lv_area_t area_to_draw = {.x1 = 0,
                                    .y1 = 0,
